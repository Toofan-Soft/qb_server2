...
class Chromosome:
    ...
    def fitness(self, df_ct, et):
        ...
        final_answer_time_prob = (1 - abs(0.6 - (chromosome_probs.p_at / et))) if (chromosome_probs.p_at / et) <= 1 else 0  # 0.6 is e_ratio

        final_topic_prob = 1.0 if topics_count() == 1 else (-sum(((counter / count) * math.log10(counter / count))
                                 for (ch, counter) in chromosome_probs.p_ch if counter != 0)) / math.log10(topics_count())

        probs = [
            final_difficulty_level_prob * 0.25,
            final_difficulty_level_r_prob * 0.225,
            final_selection_time_prob * 0.025,
            final_last_selection_prob * 0.025,
            final_answer_time_prob * 0.25,
            final_topic_prob * 0.225
        ]

        final_p = sum(prob for prob in probs)
